      ...
      await step.run(`send-email-${lead.id}`, async () => {
        try {
          ...
          if (!subjectTemplate || !bodyTemplate) {
            throw new Error(
              `Missing email template for company: ${lead.companies?.name || "Unknown"}`,
            );
          }
          ...
          const message = createEmailWithAttachment(
            ...
            subject,
            body +
              `<br/><img src="${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"}/api/track/open?id=${lead.id}" alt="" width="1" height="1" border="0" />`,
            pdfBuffer,
            ATTACHMENT_PDF,
          );
