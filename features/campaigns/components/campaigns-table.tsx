...
import { startCampaign } from "../actions/actions";
...
export function CampaignsTable({ accounts, companies }: CampaignsTableProps) {
  ...
  const filteredAndSortedCampaigns = useMemo(() => {
    ...
    return result;
  }, [campaigns, searchQuery, filterStatus, sortField, sortOrder]);
  ...
  const toggleSelectAll = () => {
    if (selectedRows.size === filteredAndSortedCampaigns.length) {
      ...
      setSelectedRows(
        new Set(filteredAndSortedCampaigns.map((c: Campaign) => c.id)),
      );
    }
  };
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent className="w-56">
              ...
              <DropdownMenuGroup>
                ...
                {["draft", "scheduled", "running", "completed"].map(
                  (status) => (
                    <DropdownMenuCheckboxItem
                      key={status}
        ...
        <table className="w-full text-sm">
          <thead className="sticky top-0 bg-gray-50/95 backdrop-blur-sm z-10">
            <tr className="border-b border-gray-200">
              <th className="w-12 px-3 py-2.5 text-left">
                <Checkbox
                  checked={
                    selectedRows.size === filteredAndSortedCampaigns.length &&
          ...
          <tbody>
            {isLoading ? (
              ...
              filteredAndSortedCampaigns.map(
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-2">
                        ...
                        >
                          <StatusIcon
                            className={`w-3 h-3 ${campaign.status === "running" ? "animate-spin" : ""}`}
                          />
