...
import { Plus, ArrowRight, ArrowLeft, Sparkles, Mail, Check, Loader2, Save } from "lucide-react"
import { createCampaign, startCampaign, type CreateCampaignInput } from './actions'
...
export function CreateCampaignModal({ accounts, companies, onSuccess }: CreateCampaignModalProps) {
                        ...
                        <div className="grid gap-4 py-4">
                            ...
                            <div className="grid gap-2">
                                ...
                                <div className="border rounded-lg p-3 max-h-40 overflow-y-auto space-y-2">
                                    {accounts.length === 0 ? (
                                        <p className="text-sm text-gray-500">No Gmail accounts connected. Go to Credentials to add one.</p>
                                    ) : (
                                        accounts.map(account => (
                                            <div key={account.id} className="flex items-center space-x-2 p-2 hover:bg-gray-50 rounded cursor-pointer" onClick={() => toggleAccount(account.id)}>
                                                <Checkbox
                ...
                {step === 2 && (
                    <>
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                Step 2: Choose which companies to target ({selectedCompanies.length} selected)
                            </DialogDescription>
                        </DialogHeader>
                        <div className="py-4">
                            ...
                            <div className="border rounded-lg p-3 max-h-60 overflow-y-auto space-y-1">
                                {companies.length === 0 ? (
                                    <p className="text-sm text-gray-500">No companies found. Upload leads with company info first.</p>
                                ) : (
                                    companies.map(company => (
                                        <div key={company.id} className="flex items-center space-x-2 p-2 hover:bg-gray-50 rounded cursor-pointer" onClick={() => toggleCompany(company.id)}>
                                            <Checkbox
                        ...
                        <div className="grid gap-4 py-4">
                            ...
                            <div className="border rounded-lg p-4 bg-gray-50/50 space-y-3">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <Label className="text-xs text-gray-500">Gmail Accounts</Label>
                                        <p className="font-medium">{selectedAccounts.length} account{selectedAccounts.length !== 1 ? 's' : ''}</p>
                                    </div>
                                    <div>
                                        <Label className="text-xs text-gray-500">Target Companies</Label>
                                        <p className="font-medium">{selectedCompanies.length} compan{selectedCompanies.length !== 1 ? 'ies' : 'y'}</p>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <p className="font-medium">
                                        ...
                                        }, 0)} leads will receive personalized emails
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="border rounded-lg p-4 bg-blue-50/30 border-blue-200">
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="flex-1">
                                        <h4 className="font-semibold text-sm text-blue-900 mb-1">AI-Generated Templates Active</h4>
                                        <p className="text-xs text-blue-700">
                                            Each company has its own personalized template with custom research and service boxes.
                                            Leads will receive their company's specific template with variables replaced.
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="grid gap-2 pt-2">
                                ...
                                <p className="text-xs text-gray-500">
                                    Leave blank to send immediately. Time is in your local timezone.
                                </p>
                            </div>
