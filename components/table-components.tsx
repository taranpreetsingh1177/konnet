...
export function TableContainer({
  ...
  return (
    <div className={cn("h-full flex flex-col bg-white", className)}>
      ...
      <div className="flex-1 overflow-auto bg-white relative">{children}</div>
      {footer}
    </div>
  );
}
...
export function TableToolbar({
  ...
  const hasActiveFilters = filterGroups.some(
    (g) => g.selectedValues.length > 0,
  );

  return (
    <div
      className={cn(
        "flex flex-col gap-4 px-4 py-3 bg-white border-b border-gray-100",
        className,
      )}
    >
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center flex-1 gap-2 overflow-x-auto no-scrollbar">
          ...
          {filterGroups.map((group) => (
            <FilterDropdown key={group.key} group={group} />
          ))}
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-48">
        ...
        <DropdownMenuRadioGroup
          value={sortBy}
          onValueChange={(val) => onSortChange(val, sortOrder || "desc")}
        >
        ...
        <DropdownMenuRadioGroup
          value={sortOrder}
          onValueChange={(val) =>
            onSortChange(sortBy || sortOptions[0]?.value, val as "asc" | "desc")
          }
        >
    ...
    <DropdownMenu>
      <DropdownMenuTrigger
        className={cn(
          ...
          "h-9 border-dashed",
          activeCount > 0 &&
            "border-solid bg-secondary/50 border-secondary-foreground/20",
        )}
      >
        ...
        {emptyState || (
          <>
            ...
            <p className="text-sm text-gray-500 max-w-sm">
              Try adjusting your search or filters, or add a new item to get
              started.
            </p>
          </>
        )}
          ...
          >
            {headerGroup.headers.map((header) => {
              return (
                <TableHead
                  key={header.id}
                  className="h-10 px-4 py-2 text-xs font-semibold text-gray-500 uppercase tracking-wider"
                >
...
export function TablePagination<T>({
  ...
  const pageSizeOptions = [
    ...
    .filter(
      (size, index, self) =>
        self.indexOf(size) === index && size <= PAGINATION.MAX_PAGE_SIZE,
    )
    .sort((a, b) => a - b);
  ...
  return (
    <div
      className={cn(
        "flex items-center justify-between px-4 py-3 border-t border-gray-100 bg-gray-50/30",
        className,
      )}
    >
